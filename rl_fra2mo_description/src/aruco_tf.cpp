...
public:
          ...
          static_broadcaster_(std::make_shared<tf2_ros::StaticTransformBroadcaster>(this))
    {
        ...
        publisher_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("/aruco_tf_map", 10);
...
private:
    void pose_callback(const geometry_msgs::msg::PoseStamped::SharedPtr pose_msg)
    {
        try
        {
            ...
            RCLCPP_INFO(this->get_logger(), "ArUco map frame: x: %.2f, y: %.2f, z: %.2f",
                        pose_in_map.pose.position.x,
                        pose_in_map.pose.position.y,
                        pose_in_map.pose.position.z);

            // Apply offset to x after logging
            pose_in_map.pose.position.x += +0.7;
            pose_in_map.pose.position.y += -1.9;
